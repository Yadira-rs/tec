...
app.get("/api/saucedemo-automate/:id", async (req, res) => {
  try {
    ...
    console.error("Error en /api/saucedemo-automate:", err);
...
app.post("/api/learn", async (req, res) => {
  try {
    ...
    console.error("Error en /api/learn:", err);
    res.status(500).json({ ok: false, error: "Error al aprender mapeo. Revisa la consola del servidor." });
  }
});
