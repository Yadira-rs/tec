...
function loadSaucedemoFrame(id) {
  // Load our local SauceDemo replica in the iframe (same-origin, no X-Frame-Options block)
  iframePlaceholder.className = "iframe-placeholder";
  iframePlaceholder.style.display = "none";
  sorianaFrame.style.display = "block";
  sorianaFrame.src = `/saucedemo-portal.html?product=${id}`;
}
...
function showFieldHints() {
  ...
  const fieldMap = currentSource === "saucedemo"
    ? { nombre_empresa:"customer", folio_orden:"order_ref", nombre_articulo:"product_name",
        precio_venta:"price", cant_solicitada:"quantity", fecha_entrega:"delivery_date",
        detalle_producto:"description" }
    : { nombre_empresa:"CustomerName", folio_orden:"PurchaseOrder", nombre_articulo:"SKUDescription",
  ...
  try {
    ...
    if (sourceData) {
      ...
      if (learned.mappings?.length) {
        ...
        await sleep(600);
        enablePhase3();
        showComparison(learned.mappings, sourceData, arcaData);
      }
    }
  } catch (_) {
...
function buildAutoOrderList() {
  ...
  const orders  = isSauce ? SAUCEDEMO_PRODUCTS : SORIANA_ORDERS;
  const total   = Object.keys(orders).length;
  const doneCount = Object.keys(orders).filter(id =>
    processed.has(id) || processed.has(id.toUpperCase())
  ).length;

  // Counter header
  const counter = document.createElement("div");
  counter.className = "ao-counter";
  counter.innerHTML = `
    <span class="ao-counter-label">Órdenes pendientes</span>
    <span class="ao-counter-badge">${doneCount}/${total} procesadas</span>
  `;
  autoOrderList.appendChild(counter);

  Object.entries(orders).forEach(([id, item]) => {
    const done = processed.has(id) || processed.has(id.toUpperCase());

    const customer = isSauce ? item.customer      : item.CustomerName;
    const product  = isSauce ? item.product_name  : item.SKUDescription;
    const price    = isSauce ? item.price          : item.UnitPrice;
    const qty      = isSauce ? item.quantity       : item.RequestedQty;
    const date     = isSauce ? item.delivery_date  : item.DeliveryDate;
    const unit     = isSauce ? "USD" : "MXN";
    const total_val = (parseFloat(price || 0) * parseFloat(qty || 0)).toLocaleString("es-MX", {
      style: "currency", currency: isSauce ? "USD" : "MXN"
    });

    const card = document.createElement("div");
    card.className = `ao-card ${done ? "ao-card-done" : "ao-card-pending"}`;

    card.innerHTML = `
      <div class="ao-card-top">
        <div class="ao-card-id">${id}</div>
        <div class="ao-card-date">📅 ${date}</div>
      </div>
      <div class="ao-card-customer">${customer}</div>
      <div class="ao-card-product">
        <span class="ao-product-name">${product}</span>
        <span class="ao-product-meta">${qty} pzs × $${price} ${unit}</span>
      </div>
      <div class="ao-card-footer">
        <span class="ao-total">Total: <strong>${total_val}</strong></span>
        ${done
          ? `<span class="state-pill processed">✓ Procesada automáticamente</span>`
          : `<button class="ao-btn" onclick="automateOrder('${id}', this)">
               <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
                 <path d="M6.5 1v2M6.5 10v2M1 6.5h2M10 6.5h2M2.93 2.93l1.41 1.41M8.66 8.66l1.41 1.41M2.93 10.07l1.41-1.41M8.66 4.34l1.41-1.41" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/>
               </svg>
               ⚡ Automatizar
             </button>`
        }
    ...
    `;
    autoOrderList.appendChild(card);
  });
}
...
async function automateOrder(po, btn) {
  ...
  try {
    ...
    if (!res.ok) {
      const errMsg = data.error || `Error ${res.status} al automatizar`;
      toast(errMsg, "error");
      addLog(`❌ ${errMsg}`, "error");
      processingAuto = false;
      btn.disabled = false;
    ...
    const cmpSource = currentSource === "saucedemo"
      ? SAUCEDEMO_PRODUCTS[po]
      : SORIANA_ORDERS[po];
    showComparison(data.mappings, cmpSource, data.values);
...
function renderRecords() {
  ...
  recordsBody.innerHTML = records.map(r => {
    ...
    return `<tr>
      ...
      <td>${r.nombre_empresa || r.cliente || "—"}</td>
...
async function clearAllRecords() {
  saveRecords([]);
  renderRecords();
  try { await fetch("/api/orders", { method: "DELETE" }); } catch (_) {}
  toast("Historial borrado", "info", 2500);
...
// ── Soriana portal → nueva orden dinámica ────────────────────────────────────
window.addEventListener("message", (e) => {
  if (e.data?.type === "soriana-new-order") {
    const { po, data } = e.data;
    if (!po || !data || SORIANA_ORDERS[po]) return;

    // Registrar en SORIANA_ORDERS local
    SORIANA_ORDERS[po] = data;

    // Agregar pill al selector de órdenes
    const shortLabel = po.slice(-3);
    const pill = document.createElement("button");
    pill.className = "order-pill";
    pill.dataset.po = po;
    pill.textContent = shortLabel;
    pill.onclick = function() { selectOrder(this); };
    document.getElementById("orderPills").appendChild(pill);

    // Si estamos en Fase 3, actualizar lista de automatización
    if (currentPhase === 3) buildAutoOrderList();

    toast(`Nueva orden ${po} detectada en Portal Soriana — disponible para automatizar`, "success", 5000);
    addLog(`📥 Nueva orden del portal: ${po} — ${data.CustomerName} — ${data.SKUDescription}`, "success");
    return;
  }

  if (e.data?.type !== "saucedemo-select") return;
  const id = e.data.id;
  if (!SAUCEDEMO_PRODUCTS[id]) return;

  selectedPO = id;
  document.querySelectorAll(".order-pill").forEach(p =>
    p.classList.toggle("active", p.dataset.po === id)
  );
  if (currentPhase >= 2) {
    updateSourceSummarySaucedemo(SAUCEDEMO_PRODUCTS[id]);
    showFieldHints();
    addLog(`Producto cambiado a ${id} desde SauceDemo portal`, "success");
  }
});
...
// ── Comparison Modal ──────────────────────────────────────────────────────────
function normalizeForCmp(v) {
  return String(v || "").toLowerCase().trim().normalize("NFD").replace(/[̀-ͯ]/g, "");
}

function valsMatchForCmp(a, b) {
  const na = normalizeForCmp(a), nb = normalizeForCmp(b);
  if (na === nb) return true;
  const numA = parseFloat(na.replace(/[^0-9.]/g, ""));
  const numB = parseFloat(nb.replace(/[^0-9.]/g, ""));
  if (!isNaN(numA) && !isNaN(numB) && numA > 0 && Math.abs(numA - numB) < 0.01) return true;
  if (na.length > 5 && nb.includes(na)) return true;
  if (nb.length > 5 && na.includes(nb)) return true;
  return false;
}

function showComparison(mappings, sourceValues, destValues) {
  if (!mappings?.length) return;
  const body = document.getElementById("comparisonBody");
  const scoreEl = document.getElementById("comparisonScore");
  if (!body || !scoreEl) return;

  let matched = 0, total = 0;

  body.innerHTML = mappings.map(m => {
    const srcKey   = m.source ?? "";
    const dstKey   = m.dest   ?? "";
    const srcLabel = m.sourceLabel || srcKey;
    const dstLabel = m.destLabel   || dstKey;

    const srcVal = String(sourceValues?.[srcKey] ?? "—");
    const dstVal = String(destValues?.[dstKey]   ?? "—");

    const hasBoth = srcVal !== "—" && dstVal !== "—";
    const isMatch = hasBoth && valsMatchForCmp(srcVal, dstVal);
    if (hasBoth) { total++; if (isMatch) matched++; }

    const badge = m.method === "observation"
      ? `<span class="cmp-badge obs">👁 Observado</span>`
      : m.method === "ai-semantic"
      ? `<span class="cmp-badge ai">🤖 IA</span>`
      : "";

    return `<tr class="${isMatch ? "cmp-row-match" : "cmp-row-miss"}">
      <td><span class="cmp-field-chip src">${srcLabel}</span>${badge}</td>
      <td class="cmp-val-cell">${srcVal}</td>
      <td class="cmp-arrow-cell">→</td>
      <td><span class="cmp-field-chip dst">${dstLabel}</span></td>
      <td class="cmp-val-cell">${dstVal}</td>
      <td class="cmp-check-cell">${isMatch ? "✅" : "❌"}</td>
    </tr>`;
  }).join("");

  const pct = total > 0 ? Math.round(matched / total * 100) : 0;
  const isPerfect = pct === 100;
  scoreEl.className = isPerfect ? "perfect" : "partial";
  scoreEl.innerHTML = `${isPerfect ? "✅" : "⚠️"}&nbsp;
    <strong>${matched} de ${total} campos exactos — ${pct}% de precisión</strong>
    ${isPerfect ? "&nbsp;· Transcripción perfecta" : "&nbsp;· Verifica los campos con ❌"}`;
